...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT ID_chercheur, Password FROM chercheuremploi WHERE Email = ?");
    $stmt->bind_param("s", $email);
    $stmt->execute();
    $stmt->store_result();
    
    if ($stmt->num_rows > 0) {
        $stmt->bind_result($id_chercheur, $password_hash);
        $stmt->fetch();
        $password_hash = password_hash($password, PASSWORD_DEFAULT);
        // Comparer le hash du mot de passe saisi avec le hash stocké en base de données
        if (password_verify($password, $password_hash)) {
            $_SESSION['Email'] = $email;
            $_SESSION['ID_chercheur'] = $id_chercheur;
...
<head>
    ...
    <title>Connexion chercheur emploi</title>
    <style>
        body {
...
<body>
    ...
    <h1>Connexion chercheur emploi</h1>
    ...
    <p>Vous n'avez pas de compte ? <a href="inscription_chercheur.html">Créez-en un</a></p>
